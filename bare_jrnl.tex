...
% \subsection{Centralized platform}

% This complexity can be mitigated by using a central platform. There are various providers of IoT platform solutions. Bringing multiple manufacturers on a common platform simplifies interoperability and data sharing tremendously. However, the platform provider is a single point of failure, may have access to all data, and is able track every interaction. Therefore the platform provider becomes a very powerful actor demanding a high level of trust. 

% \subsection{Personal data stores}

% Personal data stores are central repositories of an individual's data. A personal data store may be hosted by the individual itself or hosted with a trusted cloud service. This allows the individual to define fine grained access rights to her data. Third party requesters such as applications can request permission to particular data. Every request and transaction can be logged and access rights can be revoked. However, there is no control over data that has once been accessed by a third party. Systems like openPDS therefore employ an approach which allows to permit only certain types of queries instead of giving access to raw data. For example a health insurance provider could be granted to ask if you've walked more than 10 000 steps to grant you a discount. This leaks much less privacy sensitive information than giving access to the raw step count or even a GPS trail.

% Personal data stores give individuals the ability to have a holistic view of the data they generate and a way to manage and audit how their data is used. However, the individual's privacy can be protected even stronger. In many cases data requesters are not interested in information about individuals but in information of groups of individuals that share some common properties, e.g. people who are living in the same city.
% In the current model a requester would search for personal data stores of people living in a specific city and then query each individual data store. Thereby, the requester collects data of each individual and aggregates the data on her own machine. This unnecessary leakage of individual data can be mitigated by establishing a peer-to-peer network between the personal data stores such that computations involving data of multiple personal data stores can be executed within the network. In order to prevent owner's of personal stores and network observers to learn about transferred data, data should be encrypted and computations should be executed within a trusted computing environment.

% The main obstacle of this approach is the following chicken-and-egg problem. Consumers only demand a personal data store if devices and services support it. On the other hand device manufacturers and service providers will only provide compatibility to personal data stores if there is significant consumer demand.

% From a practical point of view given the current ecosystem the ideal systems to implement personal data stores would be routers, set-top boxes or smart home hubs.

% \subsection{Autonomous edges}

% In a sense the idea of a personal data store can be driven to it's extreme. Each individual connected device could act as its own personal data store. However heterogeneity and limited resources of devices complicate the implementation. Nevertheless, economics of scale pushes to common platforms which would make such an architecture conceivable. 

% The web of things can be seen as the simplest instantiation of this approach. Here, every connected device is a web server that exposes its resources and services as RESTful APIs. An evolution of the web of things paradigm is pursued with the machine-payable web. Cryptocurrencies enable an efficient implementation of automated nanopayments for individual API calls. This allows to bring monetary incentives to (anonymous) distributed networks, and may enable collaborative computation and data mining on a large scale.

% \subsection{Decentralized platform}

% A decentralized platform is a platform without a single point of failure. To achieve this data has to be replicated across different physical data stores controlled by different organizations. Data replication leads to high availability. However, replication of all data would lead to a multiplication of the security and privacy risk. 
...
% \begin{itemize}
% \item IETF Thing-to-Thing Research Group
% \item W3C Web of Things
% \end{itemize}


% \section{Consortia}

% \subsection{Thread Group}
% Initiated by Nest/Google for the context of the connected/smart home. Thread aims for device-to-device communications using open standards and 6LoWPAN protocols. Although there are more than 250 member companies and the certification process started in late 2015, as of writing there is no information about Thread-certified products on the market.

% The fundamental security used in the Thread Network is an elliptic curve variant of J-PAKE (ECJPAKE),
% using the NIST P-256 elliptic curve. J-PAKE is a password-authenticated key exchange
% (PAKE) with “juggling”. It uses elliptic curve Diffie-Hellmann for key
% agreement and Schnorr signatures as a NIZK (Non-Interactive Zero-Knowledge) proof
% mechanism to authenticate two peers and to establish a shared secret between them based on
% a human provided passphrase. Furthermore, a Thread network is protected with network-wide key, which is used at the MAC (Media Access Control) layer to protect the 802.15.4 MAC data frames. However, since compromising any device on the network may reveal the network-wide key to an attacker there is usually additional protection in place. 

% \subsection{Open Connectivity Foundation}

% \subsection{AllSeen Alliance}

% \subsection{OASIS}

% \subsection{Open Mobile Alliance}

% \subsection{Eclipse IoT}

% \subsection{IPSO}

% \subsection{M2M Alliance}


% \subsection{Industrial Internet Consortium}

% \subsection{Internet of Things Consortium}
